...
pub const Base = struct {
	...
	extern fn libpd_set_printhook(PrintHook) void;
	extern fn libpd_set_queued_printhook(PrintHook) void;
...
extern fn libpd_set_float(*pd.Atom, f32) void;
...
pub const BangHook = ?*const fn ([*:0]const u8) callconv(.C) void;

/// Float receive hook signature. 1st parameter is the source receiver name.
pub const FloatHook = ?*const fn ([*:0]const u8, f32) callconv(.C) void;

/// Double receive hook signature. 1st parameter is the source receiver name.
///
/// Note: only full-precision when compiled with `PD_FLOATSIZE=64`.
pub const DoubleHook = ?*const fn ([*:0]const u8, f64) callconv(.C) void;

/// Symbol receive hook signature. 1st parameter is the source receiver name.
pub const SymbolHook = ?*const fn ([*:0]const u8, [*:0]const u8) callconv(.C) void;
...
pub const ListHook = ?*const fn ([*:0]const u8, c_uint, [*]pd.Atom) callconv(.C) void;
...
pub const MessageHook = ?*const fn ([*:0]const u8, [*:0]const u8, c_uint, [*]pd.Atom) callconv(.C) void;
...
extern fn libpd_set_concatenated_printhook(PrintHook) void;

/// Set the bang receiver hook, NULL by default.
/// Note: do not call this while DSP is running
pub const setBangHook = libpd_set_banghook;
extern fn libpd_set_banghook(BangHook) void;
...
extern fn libpd_set_floathook(FloatHook) void;
...
pub const NoteOnHook = ?*const fn (c_uint, c_uint, c_uint) callconv(.C) void;

/// MIDI control change receive hook signature.
/// Channel is 0-indexed, controller is 0-127, and value is 0-127.
/// Channels encode MIDI ports via: libpd_channel = pd_channel + 16 * pd_port.
/// Note: out of range values from pd are clamped.
pub const ControlChangeHook = ?*const fn (c_uint, c_uint, c_uint) callconv(.C) void;

/// MIDI program change receive hook signature.
/// Channel is 0-indexed and value is 0-127.
/// Channels encode MIDI ports via: libpd_channel = pd_channel + 16 * pd_port.
/// Note: out of range values from pd are clamped.
pub const ProgramChangeHook = ?*const fn (c_uint, c_uint) callconv(.C) void;
...
/// Note: [bendin] outputs 0-16383 while [bendout] accepts -8192-8192.
/// Note: out of range values from pd are clamped.
pub const PitchBendHook = ?*const fn (c_uint, c_int) callconv(.C) void;

/// MIDI after touch receive hook signature.
/// Channel is 0-indexed and value is 0-127.
/// Channels encode MIDI ports via: libpd_channel = pd_channel + 16 * pd_port.
/// Note: out of range values from pd are clamped.
pub const AftertouchHook = ?*const fn (c_uint, c_uint) callconv(.C) void;

/// MIDI poly after touch receive hook signature.
/// Channel is 0-indexed, pitch is 0-127, and value is 0-127.
/// Channels encode MIDI ports via: libpd_channel = pd_channel + 16 * pd_port.
/// Note: out of range values from pd are clamped.
pub const PolyAftertouchHook = ?*const fn (c_uint, c_uint, c_uint) callconv(.C) void;

/// Raw MIDI byte receive hook signature.
/// Port is 0-indexed and byte is 0-256.
/// Note: out of range values from pd are clamped.
pub const MidiByteHook = ?*const fn (c_uint, c_uint) callconv(.C) void;


/// Set the MIDI note on hook to receive from [noteout] objects, NULL by default.
/// Note: do not call this while DSP is running.
pub const setNoteOnHook = libpd_set_noteonhook;
pub extern fn libpd_set_noteonhook(NoteOnHook) void;
...
pub extern fn libpd_set_controlchangehook(ControlChangeHook) void;
...
pub extern fn libpd_set_polyaftertouchhook(PolyAftertouchHook) void;

/// Set the raw MIDI byte hook to receive from [midiout] objects,
/// NULL by default.
/// Note: do not call this while DSP is running.
pub const setMidiByteHook = libpd_set_midibytehook;
pub extern fn libpd_set_midibytehook(MidiByteHook) void;
...
pub fn newInstance() Error!*pd.Instance {
	return libpd_new_instance() orelse Error.NewInstance;
}
extern fn libpd_new_instance() ?*pd.Instance;
...
extern fn libpd_set_instance(*pd.Instance) void;
...
pub const FreeHook = ?*const fn (?*anyopaque) callconv(.C) void;
...
extern fn libpd_set_instancedata(*anyopaque, FreeHook) void;
